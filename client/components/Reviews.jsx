...
class Reviews extends React.Component {
  ...
  changeReadmoreStatus(e, index) {
    const oldIsClicked = this.state.isClicked.slice();
    const newIsClicked = oldIsClicked.map((data, i) => {
      if (i === index) {
        return true;
      }
      return data;
  ...
  leftPageNum() {
    if (!this.props.searchBool) {
      window.scroll({ top: 1750, left: 0, behavior: 'smooth' });
      const rPN = this.state.revPageNum;
      this.setState({
        revPageNum: rPN - 1,
      });
    } else if (this.props.searchBool) {
      window.scroll({ top: 1440, left: 0, behavior: 'smooth' });
      const sPN = this.state.serPageNum;
      this.setState({
        serPageNum: sPN - 1,
      });
    }
  ...
  render() {
    const pageNum = this.props.searchBool ? this.state.serPageNum : this.state.revPageNum;
    const pageData = this.props.data.slice((pageNum-1)*7, pageNum*7);
    return (
      <Body>
